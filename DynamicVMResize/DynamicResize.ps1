Do {
    ...
    $resizeThresholdInGb = 3
    $resizeIncrementInGb = 5
    
    # Get VM subscriptionId and resourceGroup
    ...
    $query = @"
    {
    "query": "Resources | where type =~ 'Microsoft.Compute/virtualMachines' | where name == '$vmName' | project name, type, subscriptionId, resourceGroup | limit 5 | order by name asc"
    }
    "@
    ...
    {
        "properties": {
            "roleName": "Virtual Machine Disk Contributor",
            "description": "Permits the role holder to update virtual machine disks",
            "assignableScopes": [
                "/subscriptions/44363417-7132-4669-a98d-52b0dc8bd353"
            ],
            "permissions": [
                {
                    "actions": [
